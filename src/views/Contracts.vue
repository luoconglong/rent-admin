      ...
      <tbody>
        <tr v-for="c in contracts" :key="c.id">
          ...
          <td>
            ...
            <button class="btn-sm green" @click="previewAndPrint(c)">打印</button>
            <button v-if="c.status === '待签'" class="btn-sm" @click="openSign(c)">去签字</button>
    ...
    <div class="mask" v-if="showForm" @click.self="showForm = false">
      <div class="dialog">
        ...
        <div class="dialog-btns">
          ...
          <button class="btn primary" @click="createContract">生成合同</button>
        </div>
      </div>
    </div>
    ...
    <div class="mask" v-if="signing" @click.self="signing = null">
      <div class="dialog" style="width:500px">
        ...
        <div class="dialog-btns">
          ...
          <button class="btn primary" @click="saveSignature">确认签字</button>
        </div>
      </div>
    </div>
  </div>
...
const replacedClauses = computed(() => {
  ...
  const coList = d.coOccupants || d.cooccupants || []
  const coNames = coList.map(c => c.name).filter(Boolean).join('、')
  const coIdCards = coList.map(c => c.idCard).filter(Boolean).join('、')
  const totalOccupant = coNames ? (1 + coList.filter(c => c.name).length) : (d.occupantCount || 1)
  ...
  const sigHtml = sigUrl
    ? '<span style="display:inline-block;position:relative"><span style="display:inline-block;width:100px;border-bottom:1px solid #000;position:absolute;bottom:2px;left:0"></span><img src="' + sigUrl + '" style="max-width:100px;max-height:40px;position:relative;z-index:1;top:8px" /></span>'
  ...
  return contractClauses.value.split('\n').filter(c => c.trim()).map(c => {
    ...
    if (result.includes('甲方签名') && result.includes('乙方签名')) {
      result = '<span style="display:inline-block;width:48%">甲方签名：' + (replacements['{甲方}'] || '') + '</span><span style="display:inline-block;width:48%;text-align:right">乙方签名：' + sigHtml + '</span>'
    }
    if (result.includes('甲方电话')) {
      result = '<span style="display:inline-block;width:48%">甲方电话：19376884866</span><span style="display:inline-block;width:48%;text-align:right">乙方电话：' + (replacements['{乙方电话}'] || '') + '</span>'
    }
    if (result.includes('签约日期')) {
      result = '<span style="display:inline-block;width:48%">签约日期：' + (replacements['{开始日期}'] || '') + '</span><span style="display:inline-block;width:48%;text-align:right">乙方签约日期：' + (replacements['{开始日期}'] || '') + '</span>'
    }
    return result
  })
})
...
async function loadContracts() {
  ...
  if (data) {
    contracts.value = data.map(c => {
      const t = tenants.value.find(x => String(x.id) === String(c.tenant_id))
      const r = rooms.value.find(x => String(x.id) === String(c.room_id))
      return { ...c, tenantName: t?.name || '-', roomNo: r?.room_no || '-', rentAmount: c.template_data?.rentAmount || 0 }
    })
  }
}
...
function onTenantChange() {
  ...
  if (t) {
    ...
    form.startDate = t.start_date || ''
    form.endDate = t.end_date || ''
  }
}

async function createContract() {
  const t = selectedTenant.value
  if (!t) return alert('请选择租客')
  const r = rooms.value.find(x => String(x.id) === String(t.room_id))
  await supabase.from('contracts').insert({
    tenant_id: form.tenantId, room_id: t.room_id,
    template_data: {
      landlord: aptName.value && payeeName.value ? aptName.value + ' / ' + payeeName.value : '',
      tenantName: t.name,
      idCard: t.id_card || '',
      phone: t.phone || '',
      occupantCount: t.occupant_count || 1,
      coOccupants: [],
...
function previewAndPrint(c) {
  previewing.value = c
  setTimeout(function() {
    printContent = contractPrintRef.value?.innerHTML || ''
    if (printContent) {
      const win = window.open('', '_blank')
      win.document.write('<html><head><title>合同</title><style>@page{size:A4;margin:10mm}body{font-family:sans-serif;font-size:14px;line-height:2;padding:5px}h2{text-align:center;font-size:18px}p{margin:2px 0}</style></head><body>' + printContent + '</body></html>')
      win.document.close()
      setTimeout(function() { win.print(); previewing.value = null }, 500)
    }
  }, 500)
}

function doPrint() {
  const content = contractPrintRef.value?.innerHTML || printContent
  if (!content) { window.print(); return }
  const win = window.open('', '_blank')
  win.document.write('<html><head><title>合同</title><style>@page{size:A4;margin:10mm}body{font-family:sans-serif;font-size:14px;line-height:2;padding:5px}h2{text-align:center;font-size:18px}p{margin:2px 0}</style></head><body>' + content + '</body></html>')
  win.document.close()
  setTimeout(function() { win.print() }, 500)
}

async function delContract(id) {
  if (!confirm('确认删除这份合同？')) return
...
function onDraw(e) {
  ...
  ctx.beginPath(); ctx.moveTo(lastX, lastY); ctx.lineTo(e.clientX - r.left, e.clientY - r.top); ctx.stroke()
...
function onDrawTouch(e) {
  if (!isDrawing) return
  const t = e.touches[0]; const r = canvasRef.value.getBoundingClientRect()
  ctx.beginPath(); ctx.moveTo(lastX, lastY); ctx.lineTo(t.clientX - r.left, t.clientY - r.top); ctx.stroke()
...
async function saveSignature() {
  ...
  await supabase.from('contracts').update({ signature_url: url, status: '已签' }).eq('id', signing.value.id)
  signing.value = null
  loadContracts()
}
