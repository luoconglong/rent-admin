  ...
  <div class="mask" @click.self="$emit('close')">
    <div class="drawer">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:16px">
        ...
        <a :href="'?page=checkout&tenantId=' + tenant.id" style="background:#dc2626;color:white;padding:7px 14px;border-radius:8px;text-decoration:none;font-size:13px" @click="$emit('close')">退租</a>
      </div>
    </div>
  </div>
</template>
...
const emit = defineEmits(['close', 'billCreate'])
...
const pendingBills = computed(() => bills.value.filter(b => String(b.tenant_id) === String(props.tenantId) && b.status === 'pending'))
const paidBills = computed(() => bills.value.filter(b => String(b.tenant_id) === String(props.tenantId) && b.status === 'paid'))
...
function loadTenant() {
  ...
  waterReading.value = wm ? (wm.currentreading || wm.currentReading || '-') : (t.water_reading || '-')
  electricReading.value = em ? (em.currentreading || em.currentReading || '-') : (t.electric_reading || '-')
}
async function doPay(id) {
  await supabase.from('bills').update({ status: 'paid', paid_amount: bills.value.find(x => x.id === id)?.total_amount || 0, paid_time: new Date().toISOString() }).eq('id', id)
  loadAll(); loadTenant()
}
function goMeter(type) { alert('跳转到抄表页面：' + type) }
